...
runDiffbind = function(sampleCSV,samples1,samples2,plotting=FALSE,saving=TRUE,summits=FALSE,sampleName=FALSE,
                       namesOne="Condition1",namesTwo="Condition2",fromFile=FALSE,CSV=TRUE){
  samplesOnly <- read.csv(sampleCSV)
  ...
  if(saving){
    if(CSV){
      write.csv(resDF,paste(namesOne,"vs",namesTwo), row.names=FALSE)
    }
    saveRDS(resDF,paste(namesOne,"vs",namesTwo))
  }
...
runDiffbind("chipSamplesOnlyPair.csv")
...
contrastOnly = readRDS("AcVsAllCHIP.rds")
...
write.csv(sigResDF, "macs2EDGER_Ac.csv", row.names=FALSE)
